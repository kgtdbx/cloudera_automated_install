...
generate_ambari_repo()
{
			...
			sudo yum install unzip wget -y 2&>1 /dev/null
			sudo wget -O /tmp/jce_policy-8.zip http://$REPO_SERVER/repo/custom_pkgs/jce_policy-8.zip  &> /tmp/jce_download.txt
                        #This will generate internal repo file for Ambari Setup
echo "[CDH-$CDHVERSION]
name=cdh-$CDHVERSION - Updates
baseurl=http://$REPO_SERVER/repo/cloudera/cm/cloudera-manager/$OS/cloudera-manager/
gpgcheck=0
enabled=1
priority=1" > /tmp/cdh-$CDHVERSION.repo
		fi
}
...
ranger_mysql_setup()
{
        ...
        then
                ...
                $ssh_cmd  -o "StrictHostKeyChecking no" -o "CheckHostIP=no" -o "UserKnownHostsFile=/dev/null" $USER@$AMBARI_SERVER "sudo rpm -ivh http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-client-5.6.39-2.el7.x86_64.rpm http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-libs-5.6.39-2.el7.x86_64.rpm  http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-common-5.6.39-2.el7.x86_64.rpm http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql-connector-java/mysql-connector-java-5.1.37-1.noarch.rpm  http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-server-5.6.39-2.el7.x86_64.rpm   http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-libs-5.6.39-2.el7.x86_64.rpm" &> /tmp/mysql_pkginstall.txt
		wait
		# printf "systemctl  start mysqld 2>/dev/null\n/usr/bin/mysql -e \"UPDATE mysql.user SET Password=PASSWORD('$DB_ROOT_PASS') WHERE User='root';\" 2>/dev/null\n/usr/bin/mysql -e \"flush privileges;\" 2>/dev/null\n/usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"grant all privileges on *.* to 'root'@'$AMBARI_SERVER' identified by '$DB_ROOT_PASS';\" 2>/dev/null\n /usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"grant all privileges on *.* to 'root'@'$AMBARI_SERVER' with grant option;\" 2>/dev/null\n/usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"grant all privileges on *.* to 'rangeradmin'@'$AMBARI_SERVER' with grant option;\" 2>/dev/null\n/usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"grant all privileges on *.* to 'rangeradmin'@'$AMBARI_SERVER' identified by 'hadoop';\" 2>/dev/null\n/usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"grant all privileges on *.* to 'ranger'@'$AMBARI_SERVER' with grant option;\" 2>/dev/null\n/usr/bin/mysql -u root -p$DB_ROOT_PASS -e \"flush privileges;\" 2>/dev/null\n" > /tmp/commands_mysql
                ...
                $ssh_cmd  -o "StrictHostKeyChecking no" -o "CheckHostIP=no" -o "UserKnownHostsFile=/dev/null" $USER@$AMBARI_SERVER "rpm -ivh http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-client-5.6.39-2.el7.x86_64.rpm http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-libs-5.6.39-2.el7.x86_64.rpm  http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-common-5.6.39-2.el7.x86_64.rpm http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql-connector-java/mysql-connector-java-5.1.37-1.noarch.rpm  http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-server-5.6.39-2.el7.x86_64.rpm   http://"$REPO_SERVER"/repo/hdp/centos7/HDP-UTILS-1.1.0.21/mysql/mysql-community-libs-5.6.39-2.el7.x86_64.rpm" &> /tmp/mysql_pkginstall.txt
		wait
...
echo '#!/bin/bash' > /tmp/mysql.sh
echo 'if [[ "$(sudo /usr/bin/systemctl status mysqld  |grep -w Active|cut -d " " -f5)" =~ "inactive" ]]' >> /tmp/mysql.sh
echo "then" >> /tmp/mysql.sh
echo '    echo "MySQL restarted"' >> /tmp/mysql.sh
echo '    sudo /usr/bin/systemctl start mysqld' >> /tmp/mysql.sh
echo "fi" >> /tmp/mysql.sh
chmod 755 /tmp/mysql.sh

/bin/crontab -l > /tmp/"$userid"_cron
/usr/bin/echo "* * * * * sh /tmp/mysql.sh" >> /tmp/"$userid"_cron
/bin/crontab /tmp/"$userid"_cron
}
